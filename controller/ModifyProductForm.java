...
/**
 * Controller class that provides logic for the Modify Product Form of the application.
 *
 *<br><br>
 * RUNTIME ERROR occurred due to the initial implementation of the SearchPart field. The lookupPart() method is an
 * overloaded method that can accept a String or an int. Because an int will convert to a string, it is imperative
 * to determine first if the input is an integer and then if that fails, check if it is a string. Before implementing
 * the "try-catch" blocks, if a string was input, it would attempt to convert to an int, fail, and then throw an
 * exception - the method would never attempt to use the input as a String. After implementing the "try-catch" blocks
 * the application is successfully able to handle these exceptions and run the search as a String.
 *
 *
...
public class ModifyProductForm implements Initializable {
    /**
     * Product object reference used for passing an object from the MainForm to the Modify Form.
     */
    ...
    /**
     * Static method that allows the main scene to pass a selected object to the modify product scene.
     *
     * @param product The product that is being modified.
     * @param index The index of the product that is being modified.
     */
    public static void passProduct(Product product, int index) {
        ModifyProductForm.product = product;
        ModifyProductForm.index = index;
    }

    /**
     * Element of the ModifyProductForm GUI
     */
    @FXML
    ...
    /**
     * Updates the currently selected Product with the new values that have been input into the form.
    ...
    /**
     * Initializes controller, populates TableView's, and populates the text fields with the values of the selected product.
     *
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Populate text fields with corresponding values of selected product.
        AddIdTextBox.setText(String.valueOf(product.getId()));
        AddNameTextBox.setText(product.getName());
        AddInvTextBox.setText(String.valueOf(product.getStock()));
        AddPriceTextBox.setText(String.valueOf(product.getPrice()));
        AddMaxTextBox.setText(String.valueOf(product.getMax()));
        AddMinTextBox.setText(String.valueOf(product.getMin()));
